...
GDB_PY                         := $(ToolBinDir)/$(ToolPrefix)gdb-py
...
# The log table editor

LOGTABLEEDITOR		:= $(HostBinDir)/logted
LOGTABLEEDITORName	= $(notdir $(TOKENEDITOR))
ifdef StableTokens
LOGTABLEEDITORArgument := -s ${StableTokens}
endif
ifdef BuildTag
LOGTABLEEDITORArgument += -t ${BuildTag}
endif
ifdef BuildUpdateStableTokens
LOGTABLEEDITORArgument += -u
endif
...
# Gcov
# call this from the top level as $(eval $(tool-enable-gcov))

define tool-enable-gcov
ifeq ($(UseGcov), 1)
LDFLAGS   += --coverage
LANGFLAGS += --coverage -DUSE_COVERAGE
export LDFLAGS LANGFLAGS
endif
endef

# address sanitizer
# call this from the top level as $(eval $(tool-enable-asan))

define tool-enable-asan
ifeq ($(UseAsan), 1)
LDFLAGS   += -fsanitize=address
LANGFLAGS += -fsanitize=address
endif
endef
...
GccSedTool		= [^ ]\+
GccSedVers		= [0-9\.]\+
GccSedRegExp		= ^\($(GccSedTool)\) (.*) \($(GccSedVers)\).*$$
