...
ClangSedTool		= [^ ]\+
ClangSedVers		= [0-9\.]\+
ClangSedRegExp		= ^\($(ClangSedTool)\) (Ubuntu.*) \($(ClangSedVers)\).*$$
ClangSedCommand		= "s/$(ClangSedRegExp)/\2/gp"
ClangSedArgs		= $(ClangSedCommand)

ClangGrepRegExp		= ^\($(ClangVersRegExp) *$(ClangBuildRegExp)\)$$
...
GccSedVers		= .*
GccSedPlatform		= [^)]*
GccSedRegExp		= ^.* (${GccSedPlatform}) \(${GccSedVers}\)

GccSedCommand		= "s/$(GccSedRegExp)/\1/gp"
...
BinutilsSedRegExp	= ^GNU .* (.*) \(${BinutilsSedVers}\)

BinutilsSedCommand	= "s/$(BinutilsSedRegExp)/\1/gp"
BinutilsSedArgs		= $(BinutilsSedCommand)

BinutilsGrepRegExp	= 2.*
...
$(Verbose)$(LD) $(LDGCovFlag) $(LDOutputFlag) $@ $(filter-out $(DEPLIBS) $($(notdir $(basename $@))_GENERATION),$^) -Wl,--start-group $(call GenerateLibraryArguments,$(LDLIBS)) -Wl,--end-group $(call GenerateResolveArguments,$(RESLIBS)) $(LDFLAGS)
endef
