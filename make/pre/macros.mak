...
# JOBSFLAG
#
# Used to enable parallization.
# If BuildJobs isn't already set, use the number of CPUs.
ifndef BuildJobs
ifdef UseRandomBuildJobs
NumProcessors := $(shell getconf _NPROCESSORS_ONLN)
BuildJobs := $(shell bash -c 'echo $$(((RANDOM % $(NumProcessors)) + 1))')
else
...
ifneq (1, $(BuildJobs))
ifeq (0, ${MAKELEVEL})
# make sure top level make has JOBSFLAGS
MAKEFLAGS += $(JOBSFLAG)
else
# since MAKEFLAGS gets passed down to submakefiles, we don't want JOBSFLAGS
# defined in them and want to use the jobserver instead.  So, filter
# out JOBSFLAGS.
MAKEFLAGS := $(filter-out $(JOBSFLAG), $(MAKEFLAGS))
endif
endif
ifeq (1, $(BuildVerbose))
...
ResultDirectory			= $(call GenerateResultSubdirectory,$(LOGCURDIR))
...
GenerateResultPaths		= $(call CanonicalizePath,$(addprefix $(call Slashify,$(call GenerateResultSubdirectory,$(if $(1),$(1),$(LOGCURDIR)))),$(2)))
...
RelativeBuildRoot		= $(call GenerateRelativeBuildRoot,$(LOGCURDIR))
...
# GenerateBuildHeaderNames <prefixes>
#
# Scope: Private
#
# Generates build information file header names, header files use to
# indicate build target name, user, host, compiler, date, tme and
# generation.

BuildHeaderSuffix		= .h
BuildHeaderName			= build
BuildHeaderFile			= $(BuildHeaderName)$(BuildHeaderSuffix)

GenerateBuildHeaderNames	= $(if $(1),$(addsuffix $(BuildHeaderFile),$(addsuffix -,$(1))),$(BuildHeaderFile))

# GenerateBuildHeaderPaths <names>
#
# Scope: Public
#
# Generates build information file header paths, header paths use to
# indicate build target name, user, host, compiler, date, tme and
# generation.

GenerateBuildHeaderPaths	= $(call GenerateBuildPaths,$(call GenerateBuildHeaderNames,$(1)))

# GenerateReleaseHeaderNames <prefixes>
#
# Scope: Private
#
# Generates release information file header mames, header names use to
# indicate the release version.

ReleaseHeaderSuffix		= .h
ReleaseHeaderName		= release
ReleaseHeaderFile		= $(ReleaseHeaderName)$(ReleaseHeaderSuffix)

GenerateReleaseHeaderNames	= $(if $(1),$(addsuffix $(ReleaseHeaderFile),$(addsuffix -,$(1))),$(ReleaseHeaderFile))

# GenerateReleaseHeaderPaths <names>
#
# Scope: Public
#
# Generates release information file header paths, header paths use to
# indicate the release version.

GenerateReleaseHeaderPaths	= $(call GenerateResultPaths,,$(call GenerateReleaseHeaderNames,$(1)))
